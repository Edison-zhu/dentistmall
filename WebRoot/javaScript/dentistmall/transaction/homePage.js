...
function initCount(){
	$.supper("doservice", {"service" : "MdOrderInfoService.countOrderMoney","BackE" : function(jsondata) {
			if (jsondata.code == "1") {
				...
				//入库次数
				$("#WarehouseCount").html(jsondata.obj.WarehouseCount+"/");
				//入库金额
				$("#WarehouseMoney").html(toDecimal2(jsondata.obj.WarehouseMoney));
				//申领次数
				$("#ApplyCount").html(jsondata.obj.ApplyCount+"/");
				//申领金额
				$("#ApplyMoney").html(toDecimal2(jsondata.obj.ApplyMoney));
				//出库次数
				$("#ApplyCount1").html(jsondata.obj.ApplyCount+"/");
				//出库金额
				$("#ApplyMoney1").html(toDecimal2(jsondata.obj.ApplyMoney));
				//退款退货次数
				$("#RetreatCount").html(jsondata.obj.RetreatCount+"/");
				 //退款退货金额
				$("#RetreatMoney").html(toDecimal2(jsondata.obj.RetreatMoney));
				//集团统计
				$("#groupCount").html(jsondata.obj.groupCount);
				//医院统计
				$("#hospitalCount").html(jsondata.obj.hospitalCount);
				//门诊统计
				$("#departmentCount").html(jsondata.obj.departmentCount);
				//全部用户统计
			
				$("#ghdCount").html(Number(jsondata.obj.departmentCount)+Number(jsondata.obj.hospitalCount)+Number(jsondata.obj.groupCount));
			}
		}
	});
}
function initCountTransaction(){
	$.supper("doservice", {"service" : "MdOrderInfoService.countOrder","BackE" : function(jsondata) {
			if (jsondata.code == "1") {
...
function initData(){
	...
	$.supper("doservice", {"service":"MdOrderInfoService.getOrderMxListByTransaction",data:vdata, isAjax:"1", "BackE": function (jsondata) {
...
function initCountMateriel(){
	$.supper("doservice", {"service" : "MdOrderInfoService.materielState","BackE" : function(jsondata) {
		if (jsondata.code == "1") {
...
function initDataAnalysis(value){
	...
	$.supper("doservice", {"service":"MdOrderInfoService.materielAnalysis",data:vdata, isAjax:"1", "BackE": function (jsondata) {
		if (value=0) {
...
function initDataTen(){
	var vdata = "&limit="+10+"&page="+1;
	$.supper("doservice", {"service":"MdOrderInfoService.getOrderMxListmaterielTop",data:vdata, isAjax:"1", "BackE": function (jsondata) {
		 	var mxList = jsondata.rows;
		 	var str = "";
		 	if(mxList != null && mxList.length > 0){
		 		for(var i =0;i < mxList.length;i++){
...
function initDataConversion(value){
	var vdata = "&value="+value;
	$.supper("doservice", {"service":"MdOrderInfoService.materielConversion",data:vdata, isAjax:"1", "BackE": function (jsondata) {
		if (value=0) {
...
//重新提交
	$.supper("doservice", {"service":"ExportExcelService.exportSevenDayTwo","data":vdata, "BackE":function (jsondata) {
		if (jsondata.code == "1") {
