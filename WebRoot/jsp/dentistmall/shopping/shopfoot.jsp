...
<footer class="footer">
  ...
  <div class="footer-nav" style="margin-bottom:10px">
  	<c:forEach var="comm" items="${dbProList}"   begin="0"  varStatus="s" step="1">
		<a <c:if test="${comm.linkUrl!=null && comm.linkUrl!=''}">href="${comm.linkUrl}"</c:if> 
			<c:if test="${comm.linkUrl==null || comm.linkUrl==''}">href="commInfo.htm?swmId=${comm.swmId}"</c:if> >${comm.newsTitle}</a> 
	</c:forEach>
  </div>
    </c:if>
