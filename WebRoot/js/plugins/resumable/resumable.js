...
(function() {
  ...
  window.Resumable = Resumable = (function() {
    ...
    Resumable.prototype.appendFilesFromFileList = function(fileList, event) {
      ...
      for (_i = 0, _len = fileList.length; _i < _len; _i++) {
        ...
        if (file.size > 0 && !this.getFromUniqueIdentifier(this.generateUniqueIdentifier(file))) {
        	...
        	$.supper("doservice", {"service":"FileService.findBigFileCode", "data":"resumableFilename="+file.name, "BackE":function (jsondata) {
				if (jsondata.code == "1") {
					  var _mes = "";
					  if(jsondata.isEnd =='1')
						  _mes = "该文件之前已经上传完毕！是否重新上传？";
					  else
						  _mes = "该文件之前已经上传！是否重新上传？";
					  if(confirm(_mes)){
						  resumableFile = new ResumableFile(_this, file,null);
						  resumableFile._newfileName=null;
						  resumableFile._fileCode=null;
						  _this.files.push(resumableFile);
				          files.push(resumableFile);
				          _this.fire('fileAdded', resumableFile, event);
					  }else{
						  resumableFile = new ResumableFile(_this, file,jsondata.obj.filePackCount,jsondata.obj.proress);
						  resumableFile._newfileName=jsondata.obj.newFileName;
						  resumableFile._fileCode=jsondata.obj.fileCode;
						  _this.files.push(resumableFile);
				          files.push(resumableFile);
				          _this.fire('fileAdded', resumableFile, event);
				          _this.fire('fileProgress',resumableFile);
					  }
    ...
    Resumable.prototype.cancel = function() {
      ...
      //return this.fire('cancel',_fileCode);
    };
    ...
    ResumableChunk.prototype.send = function() {
      ...
      doneHandler = function(e) {
        ...
        if (status == 'success' || status == 'error') {
		  ...
		  var jsonData = $.parseJSON(_this.message());
		  _this.fileObj._newfileName = jsonData.obj.newFileName;
		  _this.fileObj. _fileCode = jsonData.obj.fileCode;
		  if(jsonData.code=='1'){
			  $('.resumable-file-'+_this.fileObj.uniqueIdentifier+' .resumable-file-progress').html(Math.floor(_this.fileObj.progress()*100) + '%');
			  return _this.resumableObj.uploadNextChunk();
		  }else if(jsonData.code=='0'){
			  return _this.resumableObj.pause();
		  }else if(jsonData.code=='2'){
			  $('.resumable-file-'+_this.fileObj.uniqueIdentifier+' .resumable-file-progress').html('100%');
			  _this.callback('setCode',jsonData.obj.fileCode,jsonData.obj.time);
			  return _this.resumableObj.uploadNextChunk();
		  }
        } else {
      ...
      if(this.fileObj._newfileName != null && this.fileObj._newfileName!=""){
    	  query = {
    		        ...
    		        proress:this.fileObj.progress(),
    		        fileCode:this.fileObj._fileCode,
    		        newfileName:this.fileObj._newfileName
    		      };
      }else{
    	  query = {
    		        resumableChunkNumber: this.offset + 1,
      ...
      this.xhr.open('POST', $.supper("getServicePath", {"service":target,"data":_data}));
  ...
  window.ResumableFile = ResumableFile = (function() {
    ...
    function ResumableFile(resumableObj,file,fileUploadCount,proress) {
      this.resumableObj = resumableObj;
      this.file = file;
      this.opt = {};
      ...
      this._error = false;
      this.chunks = [];
      this._newfileName = null;
      this._fileCode = null;
      this.bootstrap(fileUploadCount,proress);
    }
    ...
    ResumableFile.prototype.chunkEvent = function(event, message,time) {
      switch (event) {
        case "progress":
        ...
        case "setCode":
            return this.resumableObj.fire('setCode', message,time);
      }
    };
    ...
    ResumableFile.prototype.cancel = function() {
      ...
      this.resumableObj.fire('cancel',this._fileCode,this.uniqueIdentifier)
      this._newfileName = null;
      this._fileCode = null;
    ...
    ResumableFile.prototype.bootstrap = function(fileUploadCount,proress) {
      ...
      for (offset = _i = 0, _ref = max - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; offset = 0 <= _ref ? ++_i : --_i) {
    	  if(fileUploadCount != null && fileUploadCount > 0 && _i < fileUploadCount){
    		 var _resumableChunk =  new ResumableChunk(this.resumableObj, this, offset, this.chunkEvent);
    		 _resumableChunk.preprocessState=3;
    		 //_resumableChunk.status()='error';
    		  _results.push(this.chunks.push(_resumableChunk));
    		  this._prevProgress = parseFloat(proress);
    	  }else{
    		  var _resumableChunk =  new ResumableChunk(this.resumableObj, this, offset, this.chunkEvent);
     		  _results.push(this.chunks.push(_resumableChunk));
    	  }
