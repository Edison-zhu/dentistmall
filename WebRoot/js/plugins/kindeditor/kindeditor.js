...
K.options = {
	...
	htmlTags : {
		...
		span : [
			...
			'.font-weight', '.font-style', '.text-decoration', '.vertical-align', '.line-height','.padding-bottom','.text-indent'
		],
		div : [
			'id', 'class', 'align', '.border', '.margin', '.padding', '.text-align', '.color',
			'.background-color', '.font-size', '.font-family', '.font-weight', '.background',
			'.font-style', '.text-decoration', '.vertical-align', '.margin-left','.line-height','.padding-bottom','.text-indent'
		],
		table: [
			'id', 'class', 'border', 'cellspacing', 'cellpadding', 'width', 'height', 'align', 'bordercolor',
			'.padding', '.margin', '.border', 'bgcolor', '.text-align', '.color', '.background-color',
			'.font-size', '.font-family', '.font-weight', '.font-style', '.text-decoration', '.background',
			'.width', '.height', '.border-collapse','.line-height','.padding-bottom','.text-indent'
		],
		'td,th': [
			'id', 'class', 'align', 'valign', 'width', 'height', 'colspan', 'rowspan', 'bgcolor',
			'.text-align', '.color', '.background-color', '.font-size', '.font-family', '.font-weight',
			'.font-style', '.text-decoration', '.vertical-align', '.background', '.border','.line-height','.padding-bottom','.text-indent'
		],
...
_extend(KUploadButton, {
	init : function(options) {
		var self = this,
			...
			url = options.url || '',
			title =button?button.val():"",
			extraParams = options.extraParams || {},
			cls =button?(  button[0].className || ''):"",
			  att_className =options.className?options.className:"",		
		...
		var html ="";
		if(options.button_type != null && options.button_type=="singleType"){
			html = [
					'<div class="ke-inline-block ' + cls +'"  Butname="'+att_className+'"  >',
					(options.target ? '' : '<iframe name="' + target + '" style="display:none;"></iframe>'),
					(options.form ? '<div class="ke-upload-area">' : '<form class="ke-upload-area ke-form" method="post" enctype="multipart/form-data" target="' + target + '" action="' + url + '">'),
					'<span class="ke-button-common">',
					hiddenElements.join(''), 
					
					'<a class="btn btn-default btn-sm ke-button-common ke-button" id="up_'+options.button_id+'" href="#" role="button">'+options.button_title+'</a>',
					'</span>',
					'<input type="file" class="ke-upload-file" name="' + fieldName + '" tabindex="-1" id="kup_'+options.button_id+'"/>',
