!function($){
    ...
    MMPaginator.prototype = {
        _initLayout: function(){
            ...
            $limitList.on('change', function(){
                $el.data('page', 1);
                that.$mmGrid.load(that.opts.dataType, true);
            });
        ...
        , _plain: function(page, totalCount, limit){
            ...
            if(page<=1){
                ...
                $prev.find('li').prevObject.each(function (){
            ...
            $.each(list, function(index, item){
                ...
                if(item === '...'){
                    ...
                    $li.find('li').prevObject.each(function(){
                        $(this).on('click', function(){
                            $el.data('page', item);
                            that.$mmGrid.load(that.opts.dataType, true);
                        });
            ...
            //
            var $next = $('<li class="next"><div style="display: block"><a title="下一页">»</a></div></li>');
            if(page>=totalPage){
                ...
                $next.find('li').prevObject.each(function (){
                    $(this).on('click', function(){
                        $el.data('page', page+1);
                        that.$mmGrid.load(that.opts.dataType, true);
                    });
            ...
            var $goto = $('<div class="goto-page form-inline" style="margin-top: 10px;"><span>跳至</span><input type="text" class="" style="width: 40px;text-align: center;" value="" min="1" max="' + totalPage + '" /><span>页</span><button class="btn btn-primary btn-xs">跳转</button></div>');
            $goto.find('button').on('click', function(){
                var val = $goto.find('input').val();
                var pageInput = Number(val);
                if(pageInput <= 0){
                    pageInput = 1;
                } else if(pageInput >= totalPage) {
                    pageInput = totalPage;
                }
                $el.data('page', Number(pageInput));
                that.$mmGrid.load(that.opts.dataType, true);
            });
            $goto.find('input').on('keyup', function () {
                $(this).val($(this).val().replace(/[^0-9.]/g, ''));
                var val = $goto.find('input').val();
                var pageInput = Number(val);
                if(pageInput <= 0){
                    $goto.find('input').val(1);
                } else if(pageInput >= totalPage) {
                    $goto.find('input').val(totalPage);
                }
            });
            $pageList.append($goto);
        }
    ...
    $.fn.mmPaginator = function(){
        ...
        if(arguments.length === 0 || typeof arguments[0] === 'object'){
            ...
            if(option.limitList !== undefined && option.limitList.length > 0){
                options.limitList = [];
                options.limitList = option.limitList;
            }
            if (!data) {
    ...
    $.fn.mmPaginator.defaults = {
        ...
        , dataType: null  //可以修改此项，本地数据若分页，初始化传入{};可参照editMdOutOrder
    };
