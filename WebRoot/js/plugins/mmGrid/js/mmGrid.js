...
!function($){
    var MMGrid = function (element, options) {
        ...
        this._loadCount = 0;
        if(options.isSaveSel != null && options.isSaveSel==true)
        	options.cols = getControlDataGridCols(options.cols,options.name);
       
        //自定义impCode方法
        this._initRowCode($(element), options.cols);
        this.opts = options;
    ...
    MMGrid.prototype = {
    	//自定义impCode处理方法
        _initRowCode:function ($el,att_array){ 
        	   $.each(att_array,
        		        function(index_i, index_n) {
        		            if (index_n.impCode) {
        		            	 var impVal= $.supper("getsysParam",index_n.impCode);
        		            	  $el.data(index_n.impCode,impVal);
        		            }
        		        }); 
        }   
        ...
        , _initEvents: function(){
            ...
            $mmGrid.find('a.mmg-btnBackboardUp').on('click', function(){
            	if(opts.isSaveSel != null && opts.isSaveSel==true && leafCols){
                	var str="";
                    for(var colIndex=0; colIndex<leafCols.length; colIndex++){
                        if(!leafCols[colIndex].hidden)
                        	str+=leafCols[colIndex].name+",";
                    }
                    saveDataGridShowList(str,opts.name);
                }
            ...
            $body.on('click','td',function(e){
                ...
                var params = {};
                for(var i=0; i< that.opts.plugins.length; i++){
                    var plugin = that.opts.plugins[i];
                    // plugin.$el.data("page",1);
                ...
                var selData =[$.data($this.parent()[0], 'item')];
                if(!$this.parent().hasClass('selected')){
                    that.select($this.parent().index());
                    if(opts.selectedRecord[page] === undefined || opts.selectedRecord[page] === null){
                        opts.selectedRecord[page] = [];
                    }
                    opts.selectedRecord[page].push($this.parent().index());

                    if(opts.selectedRecordData[page] === undefined || opts.selectedRecordData[page] === null){
                        opts.selectedRecordData[page] = [];
                    }
                    opts.selectedRecordData[page].push(selData[0]);
                }else{
                    that.deselect($this.parent().index());
                    if(opts.selectedRecord[page] !== undefined || opts.selectedRecord[page] !== null){
                        var index = $.inArray($this.parent().index(), opts.selectedRecord[page]);
                        if(index >= 0){
                            opts.selectedRecord[page].splice(index, 1);
                        }
                    }
                    if(opts.selectedRecordData[page] !== undefined || opts.selectedRecordData[page] !== null){
                        var index = $.inArray(selData[0], opts.selectedRecordData[page]);
                        if(index >= 0){
                            opts.selectedRecordData[page].splice(index, 1);
                        }
                    }
                }
            });

            $body.on('dblclick', 'tr > td', function (e) {
                var $this = $(this);
                // var event = jQuery.Event("rowDoubleClick");
                // event.target = e.target;
                // that.$body.triggerHandler(event, [$.data($this.parent()[0], 'item'), $this.parent().index(), $this.index()]);
                if(opts.dblClickFunc !== null && opts.dblClickFunc !== undefined) {
                    opts.dblClickFunc($.data($this.parent()[0], 'item'), $this.parent().index(), $this.index()); // 参数：数据，行数，列数；行数和列数下标0开始
                }
            });
            ...
            $body.on('click','tr > td .mmg-check',function(e){
                ...
                var selData =[$.data($this.parents('tr')[0], 'item')];
                if(this.checked){
                    ...
                    opts.selectedRecord[page].push($($this.parents('tr')[0]).index());
                    ...
                    if(opts.selectedRecord[page] !== undefined || opts.selectedRecord[page] !== null){
                        var index = $.inArray($($this.parents('tr')[0]).index(), opts.selectedRecord[page]);
                        if(index >= 0){
                            opts.selectedRecord[page].splice(index, 1);
                        }
                    }
            ...
            if(opts.checkCol){
                $head.find('th .checkAll').on('click', function(){
                    ...
                    if(this.checked){
                        ...
                        var selectedIndex = that.selectedRowsIndex();
                        selectedIndex.forEach(function (value) {
                            opts.selectedRecord[page].push(value);
                        });
                        var selectedRows = that.selectedRows();
                        selectedRows.forEach(function (value) {
                            opts.selectedRecordData[page].push(value);
                        });
                    }else{
                        that.deselect('all');
                        if(opts.selectedRecord[page] !== undefined || opts.selectedRecord[page] !== null){
                            opts.selectedRecord[page].splice(0, opts.selectedRecord[page].length);
                        }
                        if(opts.selectedRecordData[page] !== undefined || opts.selectedRecordData[page] !== null){
                            opts.selectedRecordData[page].splice(0, opts.selectedRecordData[page].length);
                        }
                    }
                });
            }
        ...
        , _rowHtml: function(item, rowIndex){
            ...
            if($.isPlainObject(item)){
                ...
                for(var colIndex=0; colIndex < leafCols.length; colIndex++){
                    ...
                    trHtml.push('"');
                    if(opts.nowrap){
                        trHtml.push(' title="spanTitle' + index + '"');
                    }
                    trHtml.push('>');
                    
                    if(col.impCode){
                    	//impCode 修改 
                        var impValue=	$(this).data(col.impCode);
                        if(impValue){ 
                        	var lb_back=item[col.name];
                        	 $.each(impValue,
                                     function(index_i01, index_n01) {
                        		 if(index_n01.id==item[col.name]){
                        			 lb_back=index_n01.name;
                        			 return false;
                        		 }
	                    ...
	                    if(col.renderer){
	                        trHtml.push(col.renderer(item[col.name],item,rowIndex,col.name)); 
	                    }else{
	                        trHtml.push(item[col.name]);
	                    }
                	}
                    ...
                    trHtml.push('</span></td>');
                    if (trHtml.indexOf(' title="spanTitle' + index + '"') >= 0) {
                        var arrayIndex = trHtml.indexOf(' title="spanTitle' + index + '"');
                        if(col.renderer){
                            trHtml.splice(arrayIndex, 1);
                        }else{
                            trHtml.splice(arrayIndex, 1, ' title="' + item[col.name] + '"');
                        }
                    }
                };
                trHtml.push('</tr>');
        ...
        , _populate: function(items){
            ...
            var page = params.page;
            if(opts.selectedRecord[page] !== undefined && opts.selectedRecord[page] !== null) {
                opts.selectedRecord[page].forEach(function (value) {
                    that.select(value);
                })
            }
        }
        ...
        , _loadAjax: function(args,isSearch){
            var that = this;
            var opts = this.opts;
            var params = {};
        ...
        , _loadNative: function(args, isSearch){
            this._populate(args);
         ...
         * 加载本地数据，以对象形式保存
         */
        , _loadObject: function(args, isSearch){
            var params = {};
            for(var i=0; i< this.opts.plugins.length; i++){
                var plugin = this.opts.plugins[i];
                if(!isSearch)
            ...
            var items = null;
            if(args === null || args === undefined || JSON.stringify(args) === "{}"){
                if(this.opts.data === null || this.opts.data === undefined){
                    return;
                }
                items = this.opts.data.items;
                args = this.opts.data;
            } else {
                items = this.opts.data.items;
            }
            ...
            var endIndex = startIndex + parseInt(limit) - 1;
            ...
            this._populate(showItems);
            this._refreshSortStatus();
            this.$body.triggerHandler('loadSuccess', args);
        }
        , load: function(args,isSearch){
            var opts = this.opts;
            this._hideMessage();
            this._showLoading();
            this._loadCount = this._loadCount + 1 ;
            opts.selectedRecord.splice(0, opts.selectedRecord.length);
            opts.selectedRecordData.splice(0, opts.selectedRecordData);
            this.$head.find('th .checkAll').prop('checked','');
            if(opts.url) {
                this._loadAjax(args, isSearch);
            }else if($.isArray(args)){
                //加载本地数据
                this._loadNative(args);
            }else if ($.isPlainObject(args)){
                 this._loadObject(args, isSearch);
            }else if(opts.items){
        ...
        , selectedRecordsData: function (page) {
            if(page !== undefined && page !== null){
                if(this.opts.selectedRecordData[page] !== undefined && this.opts.selectedRecordData[page] !== null) {
                    return this.opts.selectedRecordData[page];
                }
                return [];
            }
            return this.opts.selectedRecordData;
        }
        ...
        , removeRow: function(index){
            ...
            if(index === undefined){
                ...
                if(that.opts.data != undefined && that.opts.data.items !== undefined && that.opts.data.items !== null){
                    var indexOfContain = $.inArray(item, that.opts.data.items);
                    if(indexOfContain > -1){
                        that.opts.data.items.splice(indexOfContain, 1);
                        that.opts.data.total--;
                        that.opts.data.totalCount--;
                    }
                }
    ...
    $.fn.mmGrid.defaults = {
        ...
        , dblClickFunc: null
        , selectedRecord: []
        , selectedRecordData: [] //选中的历史数据，根据页数保存
