...
import com.hsk.dentistmall.api.daobbase.imp.MdCompanyGroupDao;
...
import com.hsk.dentistmall.web.business.service.IMdDentistHospitalService;
...
@Service
public class  MdDentistHospitalService extends DSTService implements IMdDentistHospitalService {	
   /**
   ...
   */
	...
	@Autowired
	private ISysSalesmanDao sysSalesmanDao;
	@Autowired
	private IMdCompanyGroupDao mdCompanyGroupDao;
	...
	public SysRetrunMessage saveOrUpdateObject(
			MdDentistHospital att_MdDentistHospital, String listFilecode) throws HSKException {
		SysRetrunMessage srm = new SysRetrunMessage(1l);
		try {
			Integer orgid = 1;
			...
			if(listFilecode != null){
				...
				if(fileCodeArray.length >= 1 && fileCodeArray[0] != null && !fileCodeArray[0].equals("")){
					att_MdDentistHospital.setLogo(fileCodeArray[0]);
				}
			}
