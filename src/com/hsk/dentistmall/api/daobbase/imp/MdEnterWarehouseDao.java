 ...
 */
@Component
public class  MdEnterWarehouseDao extends SupperDao implements IMdEnterWarehouseDao {	
	...
	public List<Map<String,String>> getRKOrderCode(String dateStr) throws HSKDBException{
		String sql="SELECT max(RIGHT(Billcode,8)) as Billcode FROM md_enter_warehouse  WHERE 1=1";
		sql+=" AND Billcode like '"+dateStr+"%' order by Billcode desc";
		return this.getJdbcDao().query(sql);
	}

	@Override
	public MdEnterWarehouse findMdEnterWarehouseByOrderCode(String orderCode) throws HSKDBException {
		String hql = "from MdEnterWareHouse where 1=1";
		if (orderCode != null && !orderCode.equals(""))
			hql += " and relationBillCode = '%" + orderCode + "%'";
		List<MdEnterWarehouse> list = this.getHibernateTemplate().find(hql);
	...
	public void updateEnterwarehouseDeleteLogic(String addMmfIds, Integer rbaId, Integer rbsId, Integer rbbId, String purchaseType, Integer isDelete) throws HSKDBException {
		...
		String sql = "update md_enter_warehouse set is_delete = " + isDelete + " where 1=1";
		if (rbaId != null)
		...
		sql += " and wew_id in (select wew_id from md_enter_warehouse_mx where mmf_id in (" + addMmfIds + "))";
		this.getJdbcDao().execute(sql);
	}

	@Override
	public Integer getRepeatMdEnterwarehouse(MdEnterWarehouse att_mdEnterWarehouse) throws HSKDBException {
		String sql = "select wew_id from md_enter_warehouse where 1=1";
		if (att_mdEnterWarehouse.getRbaId() != null)
		...
		if (att_mdEnterWarehouse.getPurchaseType() != null && !att_mdEnterWarehouse.getPurchaseType().equals(""))
			sql += " and purchase_type = '" + att_mdEnterWarehouse.getPurchaseType() + "'";
		if (att_mdEnterWarehouse.getSuiId() != null)
			sql += " and sui_id = " + att_mdEnterWarehouse.getSuiId();
		if (att_mdEnterWarehouse.getBillcode() != null && !att_mdEnterWarehouse.getBillcode().equals(""))
			sql += " and Billcode = '" + att_mdEnterWarehouse.getBillcode() + "'";
		sql += " limit 1";
