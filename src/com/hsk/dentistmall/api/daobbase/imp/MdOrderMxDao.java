 ...
 */
@Component
public class  MdOrderMxDao extends SupperDao implements IMdOrderMxDao {	
	...
	private String getHql(MdOrderMx att_MdOrderMx) {
		...
		if (att_MdOrderMx.getSearchMatName() != null && !att_MdOrderMx.getSearchMatName().equals(""))
			sbuffer.append(" and matName like '%" + att_MdOrderMx.getSearchMatName() + "%'");
		if (att_MdOrderMx.getSearchMmfName() != null && !att_MdOrderMx.getSearchMmfName().equals(""))
			sbuffer.append(" and norm like '%" + att_MdOrderMx.getSearchMmfName() + "%'");
...
//				" LEFT JOIN md_order_after_sale_mx e on e.mom_id = a.mom_id "// and (e.after_sale != 6 and e.after_sale != 4)"
	...
	@Override
	public List<Map<Object, Object>> getMxListByOrder(MdOrderInfo att_MdOrderInfo, MdOrderMx att_MdOrderMx,String moiIds,String excludeMoiIds)throws HSKDBException {
...
//				+" WHERE a.moi_id=b.moi_id AND b.wms_mi_id=c.wms_mi_id";
		//2020 08 27 修改 yanglei  增加导出业务代理编号
		String sql="SELECT a.*,b.*,c.product_name,t5.sales_code,CONCAT(f.mmt_name,'/',e.mmt_name,'/',d.mmt_name) AS 'type_name' FROM md_order_info a LEFT JOIN md_company_group t2 ON  t2.rba_id= a.rba_id\n" +
				"\tLEFT JOIN md_dentist_hospital t3 ON t3.rbs_id = a.rbs_id\n" +
				"\tLEFT JOIN md_dental_clinic t4 ON t4.rbb_id = a.rbb_id,md_order_mx b,md_materiel_info c "
				+" LEFT JOIN md_materiel_type d ON c.md_wms_mi_id=d.mmt_id"
				+" LEFT JOIN md_materiel_type e ON c.mat_type2=e.mmt_id"
				+" LEFT JOIN md_materiel_type f ON c.mat_type1=f.mmt_id, sys_sales_man t5 "
				+" WHERE a.moi_id=b.moi_id AND ( t5.salesman_id IN ( t2.salesman_ids ) OR t5.salesman_id IN ( t3.salesman_ids ) OR t5.salesman_id IN ( t4.salesman_ids ) )  AND b.wms_mi_id=c.wms_mi_id";
		...
		if (excludeMoiIds != null && !excludeMoiIds.equals("")) {
			sql += " and a.moi_id not in (" + excludeMoiIds + ")";
		}
	...
	private String getHql(String moiIds, String searchName, Integer limit, Integer page, String orderName) {
...
//			IF (ISNULL( e.mas_id ), 0, 1 ),e.mas_id
	...
	public List<Map<String, Object>> getMxListByMoiIdsAndSearch(String moiIds, String searchName, Integer limit, Integer page) throws HSKDBException {
		String sql = getHql(moiIds, searchName, 5, 1, null);
		return this.getJdbcDao().query(sql);
	}
...
//			sql += " limit " + (page - 1) * limit + "," + limit;
//		}
		String sql = getHql(moiId.toString(), searchName, limit, page, orderName);;
		return this.getJdbcDao().query(sql);
	}
...
//				"GROUP BY e.mom_id HAVING mas_id >= mas_id or mas_id is null " +
	...
	//总的sql 主要用来导出首页7张表的数据
		...
		//导出全部订单中的模板数据
		public List<Map<String,Object>> getMxListbyOrderAndMx(MdOrderInfo att_MdOrderInfo,MdOrderMx att_MdOrderMx,String moiIds, String excludeMoiIds) throws HSKDBException{
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			String sql="SELECT  t1.order_code,t2.rba_name, t3.`rbs_name`,t4.`rbb_name`,t5.`user_name`,DATE_FORMAT(t1.Place_order_time, '%Y-%m-%d') AS TIME,";
				...
				sql+="(SELECT GROUP_CONCAT( t7.mmt_name SEPARATOR ',' ) FROM md_materiel_type t7 left join md_order_mx t6 on t7.mmt_path in (t6.mat_type) where  t6.moi_id = t1.moi_id) AS matType, ";
				...
				sql+="CONCAT('收货人',t1.Addressee,'联系电话',t1.Addressee_telephone,'收货地址',Delivery_address,'物流信息:',(express_name is not null), express_code is not NULL)AS wlxx,";				sql+="t1.enterprise_type,t1.scope_business,";
			...
			if (excludeMoiIds != null && !excludeMoiIds.equals("")) {
				sql += " and t1.moi_id not in (" + excludeMoiIds+ ")";
			}
	...
	@Override
	public List<MdOrderMx> findMdOrderMxListByOrder(MdOrderInfo mdOrderInfo) throws HSKDBException {
		String Hql= "from MdOrderMx where 1=1";
		if (mdOrderInfo.getMoiId() != null) {
			Hql += " and moiId = " + mdOrderInfo.getMoiId();
		}
		List<MdOrderMx> list=this.getHibernateTemplate().find(Hql);
		return  list;
	}
