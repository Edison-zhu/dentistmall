...
@Entity
@Table(name = "md_materiel_info" )
public class MdMaterielInfo {
///===============数据库表字段属性begin==========
			/**
			 ...
			 */
 			...
 			private String materielName; //材料
			private String ingredient; //成分
	private String productUse; //用途
	...
	private Integer mdModelId; // 对应模型库的wms_model_id

	//商品下架原因字段
	private String matRemovalReasons;
	...
	/**
		  ...
		  public void setEditDate(Date att_editDate){
			...
			@Column(name = "mat_removal_reasons" )
			public String getMatRemovalReasons() {
				return matRemovalReasons;
			}

			public void setMatRemovalReasons(String matRemovalReasons) {
				this.matRemovalReasons = matRemovalReasons;
			}
...
///===============数据库表无关子段字段属性end==========
			...
			private List<MdMaterielFormat> normList;
	...
	@Transient
	public List<MdMaterielFormat> getNormList() {
		return normList;
	}
	public void setNormList(List<MdMaterielFormat> normList) {
		this.normList = normList;
	}
	public void setNormList(HashMap normList) {
	}
	...
	@Formula("(SELECT a.file_name FROM sys_file_info a WHERE a.file_code= cfca01_filecode)")
			...
			@Formula("(SELECT a.root_path FROM sys_file_info a WHERE a.file_code= cfca01_filecode)")
			...
			@Formula("(SELECT a.file_name FROM sys_file_info a WHERE a.file_code= cfca02_filecode)")
			...
			@Formula("(SELECT a.root_path FROM sys_file_info a WHERE a.file_code= cfca02_filecode)")
			...
			@Formula("(SELECT a.file_name FROM sys_file_info a WHERE a.file_code= cfca03_filecode)")
			...
			@Formula("(SELECT a.root_path FROM sys_file_info a WHERE a.file_code= cfca03_filecode)")
			...
			@Formula("(SELECT a.file_name FROM sys_file_info a WHERE a.file_code= cfca04_filecode)")
			...
			@Formula("(SELECT a.root_path FROM sys_file_info a WHERE a.file_code= cfca04_filecode)")
			...
			@Formula("(SELECT a.file_name FROM sys_file_info a WHERE a.file_code= cfca05_filecode)") 
			...
			@Formula("(SELECT a.root_path FROM sys_file_info a WHERE a.file_code= cfca05_filecode)")
			...
			@Formula("(SELECT a.file_name FROM sys_file_info a WHERE a.file_code= cfca06_filecode)")
			...
			@Formula("(SELECT a.root_path FROM sys_file_info a WHERE a.file_code= cfca06_filecode)")
...
//			( SELECT r.mic_id FROM md_inventory_collect r  WHERE r.wms_mi_id = mdmateriel0_.wms_mi_id AND r.sui_id=2419 ) AS formula92_
			//判断收藏时的图标
//				( SELECT r.mic_id FROM md_inventory_collect r  WHERE r.wms_mi_id = mdmateriel0_.wms_mi_id AND r.sui_id=2419 ) AS formula92_
	...
	public void setMatNames(String matNames) {
	...
	public void setSecondParts(String secondParts) {
...
//	@Formula("(SELECT ROUND(SUM(b.QUANTITY),0)  FROM md_inventory_extend b WHERE b.wms_mi_id= wms_mi_id)")
	@Transient
	...
	@Formula("(SELECT b.Basic_unit  FROM md_inventory b WHERE b.wms_mi_id= wms_mi_id)")
	public String getBasiunit() {
				return Basiunit;
			}

			public void setBasiunit(String basiunit) {
				Basiunit = basiunit;
			}

			//2020-08-27 增加型号编号搜索字段
			private  String mmfCodeWmsMiId;
	@Transient
	public String getMmfCodeWmsMiId() {
		return mmfCodeWmsMiId;
	}

	public void setMmfCodeWmsMiId(String mmfCodeWmsMiId) {
		this.mmfCodeWmsMiId = mmfCodeWmsMiId;
	}
}