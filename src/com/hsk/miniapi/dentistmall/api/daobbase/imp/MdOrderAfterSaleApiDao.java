 ...
 */
@Component
public class MdOrderAfterSaleApiDao extends SupperDao implements IMdOrderAfterSaleApiDao {
    @Override
...
//            hql += " and (asState like '%" + searchAsState + "%')";
    ...
    @Override
    public PagerModel getMdOrderSaleAfterPageModelBySearch1(Integer suiId, Integer moiId, String masCode, String moCode, String serchaName) throws HSKDBException {
        String hql = "from MdOrderAfterSaleEntity where 1=1";
        if (moiId != null) {
            hql += "moiId = " + moiId;
        }
        ...
        if(serchaName != null && !serchaName.trim().equals("")){
            hql += " and (masCode like '%" + serchaName.trim().toUpperCase() + "%' or" +
                    " masId in (select a.masId from MdOrderAfterSaleMxEntity a, MdOrderMx b where a.momId = b.momId" +
                    " and b.wmsMiId in (select wmsMiId from MdMaterielInfo where matName like '%" + serchaName.trim() + "%'" +
                    " or brand like '%" + serchaName.trim() + "%' or applicantName like '%" + serchaName.trim() + "%'))" +
//                    " or masId in (select masId from MdOrderAfterSaleMxEntity where wmsMiId in (select wmsMiId from MdMaterielInfo where brand like '%" + serchaName.trim() + "%'))" +
//                    " or masId in (select masId from MdOrderAfterSaleMxEntity where wmsMiId in (select wmsMiId from MdMaterielInfo where applicantName like '%" + serchaName.trim() + "%'))" +
                    ")";
        }
    ...
    @Override
    public List<Map<String, Object>> getMdOrderSaleAfterMxList1(Integer masId, Integer moiId, String masCode, String moCode, String serchaName, Integer limit, Integer page) throws HSKDBException {
        String sql = "SELECT a.*, e.after_sale, d.mmf_code,c.root_path AS 'less_file_path', (SELECT f.param_name from sys_parameter f,sys_parameter g WHERE f.param_value= e.after_sale and f.sys_spar_id=g.spar_id and g.param_code='PAR191230112633225') as after_sale_name" +
                " FROM md_order_after_sale_mx e left join md_order_mx a on e.mom_id = a.mom_id left join md_materiel_format d on a.mmf_id = d.mmf_id ,md_materiel_info b "
                + "LEFT JOIN sys_file_info c ON b.lessen_filecode=c.file_code "
                + "WHERE a.wms_mi_id=b.wms_mi_id";
        ...
        if (serchaName != null && !serchaName.equals("")){
            sql += " and (a.mat_name like '%" + serchaName + "%'" +
                    " or a.wms_mi_id in (select wms_mi_id from md_materiel_info where brand like '%" + serchaName.trim() + "%')" +
                    ")";
        }
        sql += " order by e.mas_id";
        if(limit != null && page != null) {
    ...
    @Override
    public List<Map<String, Object>> getMdOrderSaleAfterMxListCountByMasId1(Integer masId, Integer moiId, String masCode, String moCode, String serchaName) throws HSKDBException {
        String sql = "select count(mas_mx_id) as total_count from md_order_after_sale_mx e left join md_order_mx a on e.mom_id = a.mom_id left join md_materiel_format d on a.mmf_id = d.mmf_id ," +
                "md_materiel_info b "
                + "LEFT JOIN sys_file_info c ON b.lessen_filecode=c.file_code "
                + "WHERE a.wms_mi_id=b.wms_mi_id and mas_id = " + masId;
    ...
    @Override
    public List<Map<String, Object>> getMdOrderSaleAfterMxListSuccess(MdOrderAfterSaleEntity afterSaleEntity, Object o, Integer limit, Integer page) throws HSKDBException {
        String sql = "select * from md_order_after_sale where 1=1 and as_state = 3";
//        sql += " and moi_id in ("+moiId+") order by a.mom_id";
        if(afterSaleEntity.getMasId() != null){
            sql += " and mas_id=" + afterSaleEntity.getMasId();
        }
        if (afterSaleEntity.getMoiId() != null) {
            sql += " and moi_id = " + afterSaleEntity.getMoiId();
        }
        if (afterSaleEntity.getSuiId() != null) {
            sql += " and sui_id = " + afterSaleEntity.getSuiId();
        }
        sql += " order by mas_id desc";
        if(limit != null && page != null) {
            sql += " limit " + (page - 1) * limit + "," + limit;
        }
        return this.getJdbcDao().query(sql);
    ...
    @Override
    public Integer getMdOrderSaleASMXSuccessCount(MdOrderAfterSaleEntity afterSaleEntity) throws HSKDBException {
        String sql = "select count(*) as count from md_order_after_sale where 1=1 and as_state = 3";
//        sql += " and moi_id in ("+moiId+") order by a.mom_id";
        if(afterSaleEntity.getMasId() != null){
            sql += " and mas_id=" + afterSaleEntity.getMasId();
        }
