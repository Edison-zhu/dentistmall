 ...
 */
@Component
public class MdSiteCommentApiDao extends SupperDao implements IMdSiteCommentApiDao {
	...
	@Override
	public List<MdCommentMaterielView> getMdCommentMaterielViewListBySwcId1(
			Integer swcId,Integer number) throws HSKDBException {
		String hql=" from MdMaterielInfoTemp t where 1=1";
		if (number == 10) {
			hql += " order by pyName";
		}
		if (number == 15) {
			hql += " order by mat_name";
		}
		return (List<MdCommentMaterielView>) this.getHibernatesession().createQuery(hql).setFirstResult(0).setMaxResults(5).list();
	}
