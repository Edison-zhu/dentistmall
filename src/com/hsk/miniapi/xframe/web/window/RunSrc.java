...
public class RunSrc {
	...
	public static void main(String[] args) {  
		...
		try {
			com.hsk.xframe.web.window.RunSrc.createListTableInfo();
			System.out
...
//					};
		...
		for (int i = 0; i < tableName.length; i++) {
			com.hsk.xframe.web.window.RunSrc.createTableInfo(tableName[i].toUpperCase(), pkName[i].toUpperCase(),serviceName[i],listsci);
		}
		ICodeAutoService tableSer = (ICodeAutoService) ctx
	...
	public static  void  createTableInfo(String tableName, String pkName,String serviceName,List<SysClassInfo> listsci)
			throws HSKException {
		ITableInfoService tableSer = (ITableInfoService) ctx
	    ...
	    listsci.add(com.hsk.xframe.web.window.RunSrc.createPojo(sti));
		 listsci.add(com.hsk.xframe.web.window.RunSrc.createDao(sti));
	...
	public static SysClassInfo createPojo(SysTableInfo stable) throws HSKException {
		...
		sci_init.setAuthortext(axt);
		sci_init.setVersiontext(vis);
