package com.hsk.miniapi.xframe.web.webSite.controller.imp;
...
import com.hsk.miniapi.xframe.web.webSite.controller.IKindeditorApiController;

@Controller
public class KindereditorApiController extends DSTController  implements
		IKindeditorApiController {
	private final String att_dirName = "KinderEditor";

	@RequestMapping(value = "/api/file_manager_json.htm")
	public String file_manager_json(ModelMap model, HttpServletRequest request,
		...
		if (request.getSession() != null) {
			sw = (MxxzAccount) request.getSession().getAttribute(
					SystemContext.sessionUser);
		}*/
		String updwon = "/fileInfo/updown/";
		String savePath = SystemContext.updown_File_path + "/";
		//String UserCode = (sw==null)?"admin":sw.getAccountName();
		//UserCode = UserCode == null ? "admin" : UserCode;
		savePath += this.att_dirName + "/";
		updwon += this.att_dirName + "/";
		try {
			...
			String rootUrl = request.getContextPath() + updwon + "/";
			// 图片扩展名
			...
			String msgStr = JsonUtil.getjsonstr(msg);
//			String showUrl=saveUrl.substring(saveUrl.indexOf("webapps")+7)+newFileName;
			out.println(msgStr);
		} catch (IOException e) {
	...
	@RequestMapping(value = "/api/upload_json.htm")
...
//		UserCode = UserCode == null ? "admin" : UserCode;
		...
		try {
			...
			if (!ServletFileUpload.isMultipartContent(request)) {
				return getError(out, "请选择文件。");
			}
			// 检查目录
			File uploadDir = new File(savePath);
			if (!uploadDir.isDirectory()) {
				return getError(out, "上传目录不存在。");
			}
			// 检查目录写权限
			if (!uploadDir.canWrite()) {
				return getError(out, "上传目录没有写权限。");
			}
			...
			while (itr.hasNext()) {
				...
				if (!item.isFormField()) {
					...
					if (item.getSize() > maxSize) {
						return getError(out, "上传文件大小超过限制。");
					}
					// 检查扩展名
					String fileExt = fileName.substring(
							fileName.lastIndexOf(".") + 1).toLowerCase();
					if (!Arrays.<String> asList(extMap.get(dirName).split(","))
							.contains(fileExt)) {
						return getError(out, "上传文件扩展名是不允许的扩展名。\n只允许"
					...
					try {
						...
						return getError(out, "上传文件失败。");
					}

					Map<String, Object> msg = new HashMap<String, Object>();
					msg.put("error", 0);
					msg.put("code", 1);
					String showUrl=saveUrl.replaceAll("\\\\", "/").substring(saveUrl.indexOf("webapps")+7)+newFileName;
					msg.put("url", showUrl);
					String msgStr = JsonUtil.getjsonstr(msg);
					out.print(msgStr);
					return null;
	...
	private String getError(ServletOutputStream out, String message)
			throws IOException {
		Map<String, Object> msg = new HashMap<String, Object>();
		msg.put("error", 1);
