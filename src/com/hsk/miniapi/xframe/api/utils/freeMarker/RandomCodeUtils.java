...
public  class RandomCodeUtils {  
    ...
    private static Logger logger = Logger.getLogger(com.hsk.xframe.api.utils.freeMarker.RandomCodeUtils.class);
    ...
    private static char[] dataSequence = "0123456789".toCharArray();  
    ...
    public static String getRandomData() {    
    	int index = random.nextInt(dataSequence.length);  
    	return String.valueOf(dataSequence[index]);  
    } 
    /**
     * 获取一个验证码的字符串 默认是4位，否则指定长度len的字符串
     * @param len
     * @return
     */
    public static String getRandomString(Integer len) {
		...
		StringBuilder sRand = new StringBuilder(len);    
         for (int i = 0; i < len; i++) {    
             // 取得一个随机字符    
             String tmp = getRandomChar();    
             sRand.append(tmp);  
     ...
     * @return
     */
    public static String getRandomData(Integer len) {
    	if(len==null||len==0){
    		len=CODE_NUM;
    	}
    	StringBuilder sRand = new StringBuilder(len);    
