 ...
 */
@Repository
public class SysWebsiteCommentApiDao extends SupperDao implements ISysWebsiteCommentApiDao {
	...
	@Override
	public List<SysWebsiteComment> findPagerModelSysWebsiteCommentByIdNoLogin(Integer swcId, Integer number) throws HSKDBException {
		String hql=" from SysWebsiteCommentTemp t where t.swcId= "+swcId+" and state=1 order by t.serialNumber,t.createDatetime desc";
		if(number==null){
			return (List<SysWebsiteComment>) this.getHibernatesession().createQuery(hql).list();
		}else{
			return (List<SysWebsiteComment>) this.getHibernatesession().createQuery(hql).setFirstResult(0).setMaxResults(number).list();
