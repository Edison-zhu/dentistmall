...
public class SysUserService extends DSTService implements ISysUserService {
	...
	public PagerModel getSysUserPager(SysUserInfo sysUserInfo, Integer exclude)
			throws HSKException { 
		...
		try {
			...
			if(sysUserInfo != null&& sysUserInfo.getOrgGxId() ==null){
				...
				SysUserInfo account = this.GetOneSessionAccount();
				if (account.getUserType() != null && account.getUserType() == 6 && sysUserInfo.getOrgGxId() == 1) {
					return model;
				}
				if (account.getOrganizaType().equals("0")){
					sysUserInfo.setUserType(null);
				}else if(sysUserInfo.getUserType()==null||"1".equals(sysUserInfo.getUserType().toString()) ) {
					sysUserInfo.setUserType(null);
			...
			model = sysUserInfoDao.getSysUserInfoPage(sysUserInfo, exclude);
...
//			}
		...
		try {
			...
			if (account != null && account.getSuiId() != null) {
				if (sysUserInfo.getSuiId() != null) {
					if (sysUserInfo.getOpenSecurity()!=null){
						if (sysUserInfo.getSecurityCode()==null||!sysUserInfo.getSecurityCode().equals("")){
							sysUserInfo.setSecurityCode("123456");
						}
						if (sysUserInfo.getNeedSecurity()==null){
	...
	public SysRetrunMessage updateSysUserSecurityCode(Integer suiId,Integer openSecurity,String securityCode) throws HSKException{
		...
		try{
			SysUserInfo sysUserInfo = new SysUserInfo(suiId);
			sysUserInfo = (SysUserInfo) this.getOne(sysUserInfo);
			sysUserInfo.setSecurityCode(securityCode);
			if (!Objects.equals(sysUserInfo.getOpenSecurity(), 1) && Objects.equals(openSecurity, 1))
				sysUserInfo.setSecurityCode("123456");
			sysUserInfo.setOpenSecurity(openSecurity);
			if (Objects.equals(openSecurity, 1)) {
				sysUserInfo.setNeedSecurity(1);
			}
			this.updateObject(sysUserInfo);
		}catch(Exception e){
...
//			}
		...
		public SysRetrunMessage updateHeard(String imgSrc) throws HSKException {
			...
			try{
				...
				srm.setObj(sysFileInfo.getRootPath());
