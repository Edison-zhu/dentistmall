package com.hsk.xframe.web.sysparam.service.imp;
 
import java.text.SimpleDateFormat;
import java.util.*;

import com.hsk.dentistmall.api.daobbase.IMdEnterWarehouseDao;
...
import com.hsk.xframe.api.daobbase.ISysParameterDao;
...
@Service
public class SysParameterService extends DSTService implements ISysParameterService {
	@Autowired
	private ISysParameterDao sysParameterDao;
	@Autowired
	private ISysControlParamDao sysControlParamDao;
	//用于自增物料入库
	@Autowired
	...
	public SysRetrunMessage getNewCode(String prefix) throws HSKException {
		SysRetrunMessage srm=new SysRetrunMessage(1l);
		if (prefix.equals("RKS")){
		Date newDate=new Date();
		String yearLast = new SimpleDateFormat("yy",Locale.CHINESE).format(Calendar.getInstance().getTime());
		SimpleDateFormat sdf=new SimpleDateFormat("MMdd");
		String dateStr="RK"+yearLast+sdf.format(newDate);
		try {
			List<Map<String,String>> list=mdEnterWarehouseDao.getRKOrderCode(dateStr);
			//Integer xInt=0;
			if (list.size()>0&&list!=null){
				for(Map<String, String> map:list){
					String Billcode=map.get("Billcode");
					if (Billcode!=null&&Billcode!="null"&&Billcode!=""){
						Integer BillcodeInt=Integer.parseInt(Billcode);
						Integer xInt1=BillcodeInt+1;
						String PrimaryStr=String.format("%08d",xInt1);
						dateStr+=PrimaryStr;
					}else {
						dateStr+="00000001";
					}
